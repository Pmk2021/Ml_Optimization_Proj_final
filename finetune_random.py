...
def compute_random_mask(model, density):
    masks = {}
    for name, param in model.named_parameters():
        if param.requires_grad:
            ...
            flat_mask = torch.zeros(n, dtype=torch.bool, device=param.device)

            idx = torch.randperm(n, device=param.device)[:k]
            flat_mask[idx] = True

            masks[name] = flat_mask.view_as(param)
